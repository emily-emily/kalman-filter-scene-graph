"""
SuMoTED implementation by mattmcvicar
https://github.com/mattmcvicar/SuMoTED/blob/master/SuMoTED.py
"""
...
def is_tree(A, labels, filename):
    ...
    if sorted(np.sum(A, axis=0)) == [0] + [1] * (n - 1):
        ...
        for multi_parent in np.argwhere(np.sum(A, axis=0) > 1).reshape((-1,)):
            parents = [
                labels[g] for g in np.argwhere(A[:, multi_parent]).reshape((-1,))
            ]
...
def dag_to_tree(dag):
    ...
    while len(ignore) < n:
        ...
        if len(x) > 1:
            print(
                "    - warning: found two possible spanning trees: choosing one arbitrarily"
            )
        x = x[-1]
